...
const port = 1790;
app.use(express.static(__dirname + "/trendyroot"));
app.use(
  session({
    secret: "kit2kat",
    resave: false,
...
var carts = [];
var orders = [];
var uploadfolder = path.join(__dirname, "trendyroot", "images");

connect();
getCategoriesTt();
getProductsTt();
app.get("/", async (req, res) => {
  await getCategoriesTt();
  await getProductsTt();
...
app.get("/products", async (req, res) => {
  if (!products.length) await getProductsTt();
  if (!categories.length) await getCategoriesTt();
